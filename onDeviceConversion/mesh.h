...
class WavefrontObjFile {
 public:
    explicit WavefrontObjFile(FILE* fp, std::string relativePath, bool missingMaterialsAsWhite = false) : missingMaterialsAsWhite_(missingMaterialsAsWhite) {
 ...
 private:
  ...
  void ParseFile(FILE* fp, std::string relativePath) {
    // TODO: don't use a fixed-size buffer.
    const size_t kLineBufferSize = 256;
    ...
    while (fgets(buffer, kLineBufferSize, fp) != NULL) {
      char* stripped = StripLeadingWhitespace(buffer);
      TerminateAtNewlineOrComment(stripped);
      ParseLine(stripped, line_num++, relativePath);
    }
  }

    void ParseLine(const char* line, unsigned int line_num, std::string relative_path) {
    ...
    switch (*line) {
      ...
      case 'u':
        if (0 == strncmp(line + 1, "semtl", 5)) {
          ParseUsemtl(line + 6, line_num, relative_path);
        } else {
          goto unknown;
        }
        break;
      case 'm':
        if (0 == strncmp(line + 1, "tllib", 5)) {
          ParseMtllib(line + 6, line_num, relative_path);
        } else {
          goto unknown;
        }
        break;
  ...
  void ParseSmoothingGroup(const char* line, unsigned int line_num) {
    ...
    void ParseMtllib(const char* line, unsigned int line_num, std::string relative_path) {
        
     char stringbuffer [250];
     sprintf(stringbuffer, "%s/%s", relative_path.c_str(), rtrim(StripLeadingWhitespace(line)));
     printf("\nDebug: %s\n", (stringbuffer));
        
    FILE* fp = fopen(stringbuffer, "r");
  ...
  void ParseUsemtl(const char* line, unsigned int line_num, std::string relative_path) {
    std::string usemtl;
...
//
//      char stringbuffer [250];
//      sprintf(stringbuffer, "%s/%s", relative_path.c_str(), usemtl.c_str());
//      printf("\nUse Debug: %s\n", stringbuffer);
